...
namespace Invoice
{
    public partial class OrderList : ResizeForm
    {
        ...
        public OrderList(IServiceProvider serviceProvider)
        {
            InitializeComponent();
            OrderLoad(true, true);
            AddBtns();
        ...
        public OrderList(IServiceProvider serviceProvider, bool isBuy, bool isSell, DateTime searchDate)
        {
        ...
        public void OrderLoadWithOption()
        {
            ...
            try
            {
                ...
                DS = new DataSet();
                //adapter.Fill(DS);
                OrderListView.DataSource = DS.Tables[0];
                OrderListView.AutoGenerateColumns = true;
                OrderListView.AutoResizeColumns();
                OrderListView.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            }
            catch (Exception ex)
            {
...
//" group by s.store_id, store_name, store_phone, store_address, contact_name, contact_phone, store_detail, store_fax, isMarket order by count(*) desc; ");
        ...
        {
            int orderIdx = OrderListView.Columns["Order Id"].Index;
            int invoiceIdx = OrderListView.Columns["Invoice"].Index;
