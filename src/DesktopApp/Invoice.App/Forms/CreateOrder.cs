...
namespace Invoice
{
    public partial class CreateOrder : ResizeForm
    {
        ...
        private List<Application.Dto.ProductDto> _products;
        ...
        public CreateOrder(IServiceProvider serviceProvider)
        {
            InitializeComponent();
        ...
        public CreateOrder(IServiceProvider serviceProvider, String id, OrderList ol)
        {
            InitializeComponent();
            _serviceProvider = serviceProvider;
            _productService = serviceProvider.GetRequiredService<IProductService>();
        ...
        private async void SetComboBox()
        {
            try
            {
                ...
                {
                    (orderDataView.Columns[0] as DataGridViewComboBoxColumn)!.Items.Add(product.Name);
                }
                ...
                //    };
                //    StoreList.Items.Add(comboItem);
                //}
                var stores = await _storeService.GetStores(default);
                foreach(var store in stores.Value)
                {
                    StoreList.Items.Add(new ComboboxItem
                    {
                        Text = $"{store.Id} {store.Name} (Contact: {store.ContactName})",
                        Value = store.IsMarket
                    });
                }

                StoreList.AutoCompleteMode = AutoCompleteMode.Append;
                StoreList.DropDownStyle = ComboBoxStyle.DropDownList;
                StoreList.AutoCompleteSource = AutoCompleteSource.ListItems;

                orderDataView.EditingControlShowing += 
                    new DataGridViewEditingControlShowingEventHandler(DataGridView_EditingControlShowing);
                orderDataView.CellValueChanged += 
        ...
        private async void ProductSelectionChanged(object sender, EventArgs e)
        {
        ...
        private void SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            isMarket = isMarket.Equals("1") ? true : false;
            if (isMarket)
            {
                BillToLbl.Text = "Buy From (Market): ";
            }
            else
            {
                BillToLbl.Text = "Bill To (Customer): ";
            }
        }
        ...
        private void ProdListBtn_Click(object sender, EventArgs e)
        {
            ...
            ProductCollection pc = new ProductCollection(_serviceProvider, this, products, routeValue);
